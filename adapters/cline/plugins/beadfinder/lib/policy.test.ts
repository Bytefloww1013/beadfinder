...
async function before(tool: string, args: Record<string, unknown>) {
  const hooks = plugin();
  await hooks.beforeTool({ tool, sessionID, args });
}

describe("Cline policy hooks", () => {
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "beadfinder-cline-"));
  });
  afterEach(() => {
  ...
  test("env-protection blocks .env reads", async () => {
    ...
    await expect(before("read_files", { files: [{ path: ".env" }] })).rejects.toThrow(/\[beadfinder:env-protection\]/);
  });

  test("beads-store blocks glob of beads/", async () => {
    await expect(before("glob", { pattern: "beads/**" })).rejects.toThrow(/\[beadfinder:beads-store\]/);
    await expect(before("search_codebase", { queries: ["beads/"] })).rejects.toThrow(/\[beadfinder:beads-store\]/);
  });
  ...
  test("reviewer cannot write product files", async () => {
    ...
    await expect(before("editor", { path: "src/app.ts" })).rejects.toThrow(/\[beadfinder:persona-fs-guard\]/);
  });
  ...
  test("HITL spawn is blocked", async () => {
    await expect(
      before("spawn_agent", { prompt: "Please grill this hitl ticket", description: "grill", subagent_type: "product" }),
    ).rejects.toThrow(/\[beadfinder:hitl-affinity\]/);
  });

  test("spawn-contract requires the three phrases", async () => {
    await expect(
      before("spawn_agent", { prompt: "implement login", description: "login", subagent_type: "implementer" }),
    ).rejects.toThrow(/\[beadfinder:spawn-contract\]/);
  });

  test("a complete spawn contract is allowed", async () => {
    await before("spawn_agent", {
  ...
  test("beads-only blocks gh issue create", async () => {
    await expect(before("execute_command", { command: "gh issue create --title x" })).rejects.toThrow(/\[beadfinder:beads-only\]/);
  });
  ...
  test("implementer cannot close a phase:review bead", async () => {
    ...
    st.persona = "implementer";
    saveState(dir, sessionID, st);
    fakeIssue = { id: "bf-9", status: "open", issue_type: "spawn_agent", labels: ["phase:review", "review"] };
    const spy = mockBdShow();
    try {
      await expect(before("execute_command", { command: 'bd close bf-9 --reason "done"' })).rejects.toThrow(
  ...
  test("reviewer close without the three scores is rejected", async () => {
    ...
    const spy = mockBdShow();
    try {
      await expect(before("execute_command", { command: 'bd close bf-9 --reason "looks good"' })).rejects.toThrow(
  ...
  test("reviewer close with a score below the pass bar is rejected", async () => {
    ...
    try {
      await expect(
        before("execute_command", { command: 'bd close bf-9 --reason "Review PASS: quality 7/10, correctness 9/10, pillars 9/10"' }),
  ...
  test("reviewer close with an out-of-bounds score is rejected", async () => {
    ...
    try {
      await expect(
        before("execute_command", { command: 'bd close bf-9 --reason "Review PASS: quality 999/10, correctness 9/10, pillars 9/10"' }),
  ...
  test("reviewer may close a non-review bead without scores", async () => {
    ...
    st.persona = "reviewer";
    saveState(dir, sessionID, st);
    fakeIssue = { id: "bf-8", status: "open", issue_type: "spawn_agent", labels: ["phase:execute"] };
    const spy = mockBdShow();
    try {
      await before("execute_command", { command: 'bd close bf-8 --reason "done"' });
    } finally {
      fakeIssue = null;
      spy.mockRestore();
    }
