...
export async function refreshAndInject(
  ctx: ClineContext | undefined,
  sessionID: string,
  force = false,
): Promise<string> {
...
export async function bootSession(ctx: ClineContext | undefined, sessionID: string): Promise<void> {
  const cwd = getCwd(ctx);
...
export function maybeRecordPersona(cwd: string, sessionID: string, agent: string | undefined): SessionState {
  const state = loadState(cwd, sessionID);
...
export async function maybeYield(cwd: string, sessionID: string, why: string): Promise<void> {
  const st = loadState(cwd, sessionID);
...
export function compactContext(cwd: string, sessionID: string): string[] {
  const state = loadState(cwd, sessionID);
  return [
...
export function createBeadfinder(ctx?: ClineContext) {
  ...
  return {
    ...
    afterRun: async (input: { sessionID?: string; sessionId?: string; reason?: string }) => {
      if (hooksDisabled()) return;
      const sessionID = input.sessionID || input.sessionId || "default";
      const reason = input.reason || "run.completed";
      await maybeYield(cwd, sessionID, reason);
    },
    ...
    beforeTool: async (input: {
      ...
      if (
        debugEnabled(cwd) &&
        (isBashTool(name) || isWriteTool(name) || isSpawnTool(name))
      ) {
        debugLog(cwd, {
          level: "info",
          source: "hook",
    ...
    afterTool: async (input: {
      ...
      const sessionID = input.sessionID || input.sessionId || "default";
      const args = (input.args || input.input || input.parameters || {}) as Record<string, unknown>;
      const rawOutput = input.output !== undefined ? input.output : (input.result !== undefined ? input.result : (input.error !== undefined ? input.error : ""));
      const text = typeof rawOutput === "string" ? rawOutput : JSON.stringify(rawOutput || "");
      const isErr =
        Boolean(input.isError) ||
        Boolean(input.error) ||
        (/\[beadfinder:/.test(text)
          ? false
          : /(?:^|\n)\s*(?:error|failed|permission denied)/i.test(text));
      await handleToolAfter(ctx, sessionID, name, args, text, isErr);
    },
  };
}
